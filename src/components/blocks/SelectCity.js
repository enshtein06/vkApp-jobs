...
import { Panel, View, PanelHeader, Search } from "@vkontakte/vkui";
...
class SelectCity extends PureComponent {
  ...
  renderHeaderButton = <HeadButton onClick={this.goToMain} />;
  ...
  renderCell = ({ key, index, style }) => {
    ...
    return (
      ...
      >
        {title}
        {this.props.selectedCity.id === id && (
          <div className="cityCell__arrow">
            <Icon24Done />
          </div>
        )}
      </div>
